...
(function () {
  ...
  // -------------------------------------------------------------
  // SEP → ONEXUS auto converter
  // Detects SEP solver JSON and returns ONEXUS graph structure
  // -------------------------------------------------------------
  function tryConvertSepToOnexus(data) {
    // Must look like { solutions: [...] }
    if (!data || !Array.isArray(data.solutions) || data.solutions.length === 0)
      return null; // not SEP

    const sol = data.solutions[0];
    if (!sol?.structure?.components) return null;

    // Convert nodes
    const nodes = (sol.structure.components || []).map(c => ({
      data: {
        id: c.id,
        nodeType: "Element",
        category: c.type, // SEP type becomes ONEXUS category
        label: {
          en: `${c.type} (${c.id})`,
          jp: `${c.type}（${c.id}）`
        },
    ...
    // Type mapping from SEP to ONEXUS
    const typeMap = {
    ...
    // Convert edges
    const edges = (sol.structure.relationships || []).map(r => {
      const mapped = typeMap[String(r.type).toLowerCase()] || r.type;
      const bidir = !!r.properties?.bidirectional;
      ...
      return {
        data: {
          ...
          source: r.source_id,
          target: r.target_id,
          directional: !bidir,       // SEP bidirectional → ONEXUS no arrow
  ...
  function loadJSON(event) {
    ...
    reader.onload = (e) => {
      ...
      // Auto convert SEP → ONEXUS if needed
      let graph = tryConvertSepToOnexus(data) || data;

      const { valid, errors } = validateOnexusJson(graph);
      if (!valid) {
        alert("Schema errors:\n" + errors.join("\n"));
        return;
      }
